...
# TODO: purge data before filling csv files, so I don't have to manually do it
with open('datasets/creditcard.csv', 'r') as csvDataFile:
    with open('datasets/test_data.csv', 'a') as testFile:
        with open('datasets/training_data.csv', 'a') as trainingFile:
            csvReader = csv.reader(csvDataFile)
            training_writer = csv.writer(trainingFile, delimiter=',', lineterminator='\n')
            ...
            for row in csvReader:
                ...
                # TODO: Make sure to evenly distribute the fraudulent transaction
                # TODO: Make sure not to over populate one list more than the other
                if rand_val <= 7:
                    if is_fraud == '1':
                        train_fraud_count += 1
                    training_count += 1
